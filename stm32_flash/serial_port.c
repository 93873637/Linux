			       ...
			       const serial_stopbit_t stopbit)
{
	...
	h->newtio.c_cc[VMIN] = 50;
	h->newtio.c_cc[VTIME] = 10;	/* in units of 0.1 s */
									...
									size_t nbyte, size_t *nread)
{
	...
	if (r < 0) {
		printf("serial read fail r %d\n", r);
		return PORT_ERR_UNKNOWN;
	} else {
		*nread = r;
	}

	printf("serial read %x = 0x%x len %d\n", &(pos[3]), pos[3], *nread);
#if 0
