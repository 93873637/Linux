...
def tile_quad(windows_list, monitors_areas, dws):
    ...
    number_of_windows = len(windows_list[0])
    number_of_windows2 = len(windows_list[1])
    print ("###@: number_of_windows=" + str(number_of_windows))
    print ("###@: number_of_windows2=" + str(number_of_windows2))
    if number_of_windows >= 2:
...
def tile_vertically_on_area(windows_list, x, y, w, h, dws):
    ...
    for checked_window in windows_list:
        support.moveresize(checked_window, x, y + win_num*step + glob.screen_margin_top, w , step, dws)
        win_num += 1
...
def tile_horizontally_on_area(windows_list, x, y, w, h, dws):
    ...
    win_num = 0
    for checked_window in windows_list:
        support.moveresize(checked_window, x + win_num*step + glob.screen_margin_start, y, step, h, dws)
        win_num += 1
